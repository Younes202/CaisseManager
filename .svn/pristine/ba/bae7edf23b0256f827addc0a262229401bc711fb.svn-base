...
<%@page import="appli.controller.domaine.caisse.action.CaisseBaseAction"%>
...
List<ArticlePersistant> listArticleFavoris = (List<ArticlePersistant>)request.getAttribute("listArticleFavoris");
	...
	<%
		for(ArticlePersistant articleP : listArticleFavoris){
			...
			%>
			<std:linkPopup style='<%=CaisseWebBaseAction.GET_STYLE_CONF("BUTTON_ARTICLE", "COULEUR_TEXT_DETAIL")%>' action="caisse-web.balance.addArticleFamilleCmd" params='<%=params%>' workId="<%=articleP.getId().toString()%>" classStyle="caisse-btn detail-article-btn" value="">
