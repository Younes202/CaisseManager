...
<%
	...
	%>
<div style="position: fixed;bottom: 35px;right: 122px;z-index: 1000;" id="pager-bloc">
	...
	<span style="color: black;font-weight: bold;"><%=currNbrElmnt%>/<%=nbrTotal%></span>
...
%>
	...
	%>     
            <div class="plan" style="width: 100%;float: left;margin-left: 0px;">
                <table style="width: 100%;background-color: <%=colors[idx]%>;border: 1px solid gray;">
                	 <tr style="border-bottom: 1px dashed blue;height: 30px;">
                	 	<td style="width: 15px;">
	                		<%
	                			if(tp.equals("pre")){
	                		%>
	                		<input type="checkbox" name="mvm-check" value="<%=mvm.getId()%>" <%=StringUtil.contains(mvm.getId().toString(), checks)?" checked='checked'":""%> style="height: 24px;width: 24px;">
		                	<%
		                		}
		                	%>
                		</td>
                	...
                	<tr style="border-radius: 4px;cursor:pointer;border-bottom: 1px dashed blue;">
                		<td colspan="2" style="text-align: center;">
                			...
                			<span style="color: blue;font-weight: bold;font-size: 12px;"><%=typeCmd.toUpperCase()%></span>
                		</td>
                	 </tr>
                ...
                <table style="width: 100%;color: black;text-align: left;" id="cmd-tablexxxx">
					<%
					         	...
					         	<tr style="height:28px;<%=styleAnnul%>" class="<%=classType%>" par="<%=params%>">
						       		<td colspan="2" style="<%=styleTd%>">
										...
										%>
						       			...
						       			<%=(!isNotArt ? "<span class='fa fa-circle' style='color:#9dc2ff;'></span>&nbsp;":"&nbsp;")+libCmd%>
					         	...
					         	<tr style="<%=styleAnnul%><%=!isNotArt ? "height:28px;":""%>" class="<%=classType%>" par="<%=params%>">
						       		...
						       		<div style="vertical-align: middle;">
						       			...
						       			<% } else{%>
							       			<std:link action="" targetDiv="XXX" classStyle="btn btn-palegreen btn-xs" icon="fa fa-fire" style="visibility:hidden;position:absolute;top:-100px;" />
							       		<%}%>
							       		<span style="font-weight: bold;padding-right: 3px;"><%=qte %></span>
							       	  <%}%>
							       	  </div>
						       		</td>
						       </tr>
