...
//		BigDecimal tauxMarge = societeLivrP.getTaux_marge();
	...
	//--------------------------------- SITUATION ERP ------------------------------------
		@Override
		public Map getSituationClientErp(RequestTableBean cplxTable, Long clientId, Date dateDebut, Date dateFin){
			Map mapData = new HashMap<>();
			mapData.put("dtStart", dateDebut);
			mapData.put("dtEnd", dateFin);
			
			// Table pagination --------------------------------------------------
			if(cplxTable != null){
				String req = "from MouvementPersistant mouvement where "
				+ "mouvement.date_mouvement >='{dateDebut}' "
				+ "and mouvement.date_mouvement <='{dateFin}' "
				+ "and mouvement.opc_client.id='{clientId}' "
				...
				List<MouvementPersistant> listMouvement = (List<MouvementPersistant>) findByCriteria(cplxTable, req, false);
				mapData.put("data", listMouvement);
			}
			
			List<MouvementPersistant> listMvm = getQuery("from MouvementPersistant where "
					+ "date_mouvement >=:dateDebut "
					+ "and date_mouvement <=:dateFin "
					+ "and opc_client.id=:clientId "
					+ "and (is_annule is null or is_annule=0) ")
			...
			int nbrCmd = 0;
			Map<String, BigDecimal> mapDetPaiement = new HashMap<>();
			for(MouvementPersistant mvm : listMvm){
				for(PaiementPersistant paiement : mvm.getList_paiement()) {
					String mode_paiement = "paie_"+paiement.getLibelle();
					mapDetPaiement.put(mode_paiement, BigDecimalUtil.add(mapDetPaiement.get(mode_paiement), paiement.getMontant()));
					mttTotal = BigDecimalUtil.add(mttTotal, paiement.getMontant());
				}
				nbrCmd++;
			}
			// [Solde recharge, solde mouvement]
		...
		@Override
		public Map getSituationSocieteLivrErp(RequestTableBean cplxTable, Long societeLivrId, Date dateDebut, Date dateFin){
			Map mapData = new HashMap<>();
			mapData.put("dtStart", dateDebut);
			mapData.put("dtEnd", dateFin);
			...
			// Table pagination --------------------------------------------------
			if(cplxTable != null){
				String req = "from MouvementPersistant caisseMouvement where "
				+ "date_mouvement >='{journeeDebutId}' "
				+ "and date_mouvement <='{journeeFinId}' "
				+ "and caisseMouvement.opc_societe_livr.id='{societeLivrId}' "
				+ "and (caisseMouvement.is_annule is null or caisseMouvement.is_annule=0) "
				+ "order by caisseMouvement.id desc";
				Map<String, Object> formCriterion = cplxTable.getFormBean().getFormCriterion();
		...
		@Override
		public Map getSituationLivreurErp(RequestTableBean cplxTable, Long livreurId, Date dateDebut, Date dateFin){
			Map mapData = new HashMap<>();
			mapData.put("dtStart", dateDebut);
			mapData.put("dtEnd", dateFin);
