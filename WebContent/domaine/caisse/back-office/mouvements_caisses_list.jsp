...
$(document).ready(function (){
	...
	$(document).on('click', 'a[id="lnk_annul"]', function(){
		showConfirmDeleteBox('<%=EncryptionUtil.encrypt("caisse.journee.annulerCommande")%>', $(this).attr("params"), $(this), "Cette commande sera annulée ainsi que son stock. Cette opération n''est pas annulable.<br>Voulez-vous confirmer ?", null, "Annulation commande");
    });
});
...
<div class="page-breadcrumbs breadcrumbs-fixed">
	<ul class="breadcrumb">
		...
		<li>Gestion de la journé</li>
		<li class="active">Mouvements journée</li>
	</ul>
</div>
<!-- /Page Breadcrumb -->
<!-- Page Header -->
<div class="page-header position-relative">
	<div class="header-title" style="padding-top: 4px;">
		...
		<%}  
		if(StringUtil.isTrue(ContextGloabalAppli.getGlobalConfig("DOUBLE_CLOTURE")) 
				&& StringUtil.isTrue(""+request.getAttribute("isDoubleCloture")) 
				&& isJourneeOuverte 
				&& !isDetailJourneeDroit){ %> 
				<std:link actionGroup="C" classStyle="btn btn-danger" action="caisse.caisse.init_cloturer_shifts" tooltip="Double cl&ocirc;ture des shifts activée" value="Double cl&ocirc;ture shifts" />
	      	<% }
			} else{ %>
		...
		<div class="col-lg-12 col-sm-12 col-xs-12">
			<div class="widget">
	      	...
	      	<%if(ContextAppli.getUserBean().getIs_RemoteAdmin()){ %>
	      		<std:link classStyle="btn btn-danger" action="caisse.journee.deplace_cmds" value="Déplacer vers shift" icon="fa fa-3x fa-random" tooltip="Déplacer les mouvements vers un autre shift" />
				<std:link classStyle="btn btn-danger" action="caisse.journee.recalculChiffresMvmJournee" value="Re-calcul Mouvements" tooltip="Recalcul mouvements journée : total articles et total mouvement" />
	      	<%} %>
									...
									<div class="row">
								        <div class="form-group">
								            ...
								            <div class="col-md-2" style="text-align: center;">
								            	<std:link action="caisse.caisse.find_mouvement" params="prev=1&is_fltr=1" icon="fa fa-arrow-circle-left" tooltip="Journée précédente" />
								            	<std:link action="caisse.caisse.find_mouvement" params="next=1&is_fltr=1" icon="fa fa-arrow-circle-right" tooltip="Journée suivante" />
								            </div>
								...
								<cplx:table name="list_caisseMouvement" transitionType="simple" width="100%" titleKey="caisseMouvement.list" initAction='<%=ControllerUtil.getMenuAttribute("isCaisse", request) == null ? "caisse.journee.find_mouvement":"caisse.caisse.find_mouvement"%>'
									...
									<cplx:header>
										...
										<% }%>
										<cplx:th type="date" value="Date" field="caisseMouvement.date_creation" width="120" />
										...
										<cplx:th type="long[]" value="Employé" field="caisseMouvement.opc_employe.id"  groupValues="${listEmploye }" groupKey="id" groupLabel="nom" filterOnly="true"/>
										<cplx:th type="long[]" value="Client" field="caisseMouvement.opc_client.id" groupValues="${listClient }" groupKey="id" groupLabel="nom" filterOnly="true" />
										<% if(isDetailJourneeDroit){ %>
											<cplx:th type="decimal" value="Mtt cmd" field="caisseMouvement.mtt_commande" width="70" />
											<cplx:th type="decimal" value="Mtt cmd net" field="caisseMouvement.mtt_commande_net" width="70" />
											<cplx:th type="decimal" style="color:red;" value="Mtt. annulation" field="caisseMouvement.mtt_annul_ligne" width="70" />
											<cplx:th type="decimal" value="Cmd Réduction" field="caisseMouvement.mtt_reduction" width="70" filtrable="false" />
											<cplx:th type="decimal" value="Art Réduction" field="caisseMouvement.mtt_art_reduction" width="70" filtrable="false" />
											<cplx:th type="decimal" value="Mtt offert" field="caisseMouvement.mtt_art_offert" width="70" filtrable="false" />
										<% } %>
										<cplx:th type="string[]" value="Type" field="caisseMouvement.type_commande" width="90" groupValues="${typeCmd }" />
										<cplx:th type="string[]" value="Mode paiement" field="caisseMouvement.mode_paiement" width="100" groupValues="${modePaie }"/>
										<% if(ControllerUtil.getMenuAttribute("isCaisse", request) == null){ %>
									...
									<cplx:body>
										...
										<%if(ControllerUtil.getMenuAttribute("journeeId", request) == null){ %>	
											<c:if test="${oldjour == null  or oldjour != caisseMouvement.opc_caisse_journee.opc_journee.id }">
												<tr>
													<td colspan="${isDetailJourneeDroit? '9':'15'}" noresize="true" class="separator-group">
										...
										<%} %>
											<!-- Caisse journée -->
										     <c:if test="${oldjourCaisse == null  or oldjourCaisse != caisseMouvement.opc_caisse_journee.id }">
												<tr>
													<td colspan="${isDetailJourneeDroit? '9':'15'}" noresize="true" style="font-size: 13px;font-weight: bold;color:blue;background-color: #fff9e0;padding-left:30px;">
														Shift de <fmt:formatDate value="${caisseMouvement.opc_caisse_journee.date_ouverture}" pattern="HH:mm:ss"/>
														<%if(isModeDeplace){%>
															<std:select style="font-size: 10px;height: 27px;margin-top: 2px;background-color: #ffb03a;" name="journee_target" type="long" mode="std" data="${listJournee }" key="id" labels="date_journee" />
															<std:link action="caisse.journee.deplacerShifts" params="cjId=${caisseMouvement.opc_caisse_journee.id }" value="Déplacer vers cette journée" tooltip="Déplacer vers cette journée" />
														<%} %> 
													</td>
												</tr>	
											</c:if>
											...
											<cplx:tr workId="${caisseMouvement.id }">
												...
												<cplx:td>
													<fmt:formatDate value="${caisseMouvement.date_creation }" pattern="dd/MM/yyyy HH:mm" />
												</cplx:td>
												...
												<%if(isDetailJourneeDroit){%>
													...
													<cplx:td align="right" style="color:#f600b7;" value="${caisseMouvement.mtt_reduction}"></cplx:td>
													<cplx:td align="right" style="color:#f600b7;" value="${caisseMouvement.mtt_art_reduction}"></cplx:td>
													<cplx:td align="right" style="color:#f600b7;" value="${caisseMouvement.mtt_art_offert}"></cplx:td>
												<%}%>
												...
												<cplx:td align="right">
													<%if(ControllerUtil.getMenuAttribute("isCaisse", request) == null){ 
														...
														<%} %>
														<std:linkPopup actionGroup="S" classStyle="btn btn-sm btn-default shiny" style="color:black" action="stock-caisse.mouvementStock.init_pdf_facture" workId="${caisseMouvement.id}" icon="fa fa-file-pdf-o" tooltip="Télécharger facture pdf" />
													<%} %>
										...
										<c:if test="${!list_caisseMouvement.isEmpty()}">
												...
												<tr class="sub">
														<%if(ControllerUtil.getMenuAttribute("journeeId", request) == null){%>
															<td colspan="5"></td>
														<%} else{ %>
															<td colspan="<%=isModeDeplace ? "7":"6"%>"></td>
														<%} %>
														<td align="right">
															<span style="font-size: 12px !important;font-weight: bold;height: 28px;" class="badge badge-blue">
														...
														<td align="right">
															<span style="font-size: 12px !important;font-weight: bold;height: 28px;color:red;" class="badge badge-blue">
								...
								<div class="row">
								    <div class="form-group">
									    ...
									    <div class="col-md-10">
											<std:select name="shift_target" type="long" mode="std" data="${listShift }" key="id" labels="opc_journee.date_journee;' [';date_ouverture;'=>';date_cloture;']'" />
