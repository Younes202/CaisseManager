...
<%if(isDetailJourneeDroit){ %>	      
	      ...
	      <std:link actionGroup="C" params="isM=1" action="caisse.journee.chiffre_employe" icon="fa fa-users" style="color: #3F51B5;" value="Chiffres employés" />
<%} %>	      
      </div>
      <!--Header Buttons-->
...
<!-- row -->
	...
	<cplx:table name="list_journee" transitionType="simple" width="100%" forceFilter="true" titleKey="journee.list" initAction="caisse.journee.work_find" checkable="false" autoHeight="true">
		...
		<%if(isDetailJourneeDroit){ %>		
			...
			<cplx:th type="decimal" value="Cmd Réduc" field="journee.mtt_reduction" width="60" />
			<cplx:th type="decimal" value="Art Réduc" field="journee.mtt_art_reduction" width="60" />
			<cplx:th type="decimal" value="Offert" field="journee.mtt_art_offert" width="60" />
			<cplx:th type="decimal" value="Cmd Annul" style="color:red;" field="journee.mtt_annule" width="70" />
			<cplx:th type="decimal" value="Art Annul" style="color:red;" field="journee.mtt_annule_ligne" width="70" />
		...
		<cplx:body>
			<c:forEach items="${list_journee }" var="journee">
				<cplx:tr workId="${journee.id }" style="font-size: 12px;">
					...
					<cplx:td style="color:blue;" value="${journee.date_journee}"></cplx:td>
				...
				<%if(isDetailJourneeDroit){ %>
					...
					<cplx:td style="font-weight:bold;" align="right">
					
						<%if(isMttJrnEnNet){ %>
							<fmt:formatDecimal value="${bigDecimalUtil.substract(journee.mtt_total_net, journee.mtt_portefeuille, journee.mtt_donne_point)}" />
						<%} else{ %>
							<fmt:formatDecimal value="${journee.mtt_total_net}" />
						<%} %>
					...
					<cplx:td align="right" style="color:green;" value="${bigDecimalUtil.isZero(journee.mtt_art_reduction)?null:journee.mtt_art_reduction }"></cplx:td>
				...
				<%} %>				
					<cplx:td align="left">
						...
						<c:choose>
							<c:when test="${journee.statut_journee =='C' }">
								<li>
									<std:link action="caisse.journee.reOuvrirJournee" icon="fa fa-unlock" classStyle="" workId="${journee.id }" value="Ré-ouvrir la journée" tooltip="Ré-ouvrir la journée" />
								</li>	
							</c:when>
							<c:when test="${not empty lastJournee and lastJournee.id!=journee.id }">
								<li>
									<std:linkPopup action="caisse.journee.init_cloture" icon="fa fa-lock" classStyle="" params="isReclos=1" workId="${journee.id }" value="Clore la journée" tooltip="Clore la journée" />
								</li>	
							</c:when>	
						</c:choose>
						<%if(StringUtil.isTrue(ContextGloabalAppli.getGlobalConfig("DOUBLE_CLOTURE")) 
							...
							<li>
								<std:link actionGroup="C" params="jrn=${journee.id }" icon="fa fa-retweet" classStyle="" style="color:red;" value="Double lôture" action="caisse.caisse.init_cloturer_shifts" tooltip="Double clôture des shifts activée" />
							</	
      					<%} %>
						<%} %>
							 <li>
						 		<std:linkPopup actionGroup="C" classStyle="" style="color:blue;" action="caisse.journee.edit_date_journee" workId="${journee.id }" value="Changer date" params="init=1" icon="fa fa-edit" tooltip="Changer la date de la journée" />
  							  </li>	
						 	<li>
						 		<std:link actionGroup="D" tooltip="Supprimer" classStyle="" value="Supprimer" action="caisse.journee.work_delete" style="color:red;" icon="fa fa-trash-o" workId="${journee.id }" params="curTbl=list_journee&nosave=true"/>						 	
						 	</li>
						 </ul>	
					</cplx:td>
				</cplx:tr>
