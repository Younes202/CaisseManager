		...
		<cplx:body>
			...
			%>
				...
				<c:if test="${article.familleStr.size() > 0}">
					<c:forEach var="i" begin="0" end="${article.familleStr.size()-1}">
						<c:if test="${empty oldfam or i>(oldfam.size()-1) or article.familleStr.get(i).code != oldfam.get(i).code}">
						     <tr>
								<td colspan="5" noresize="true" class="separator-group" style="padding-left: ${article.familleStr.get(i).level*10}px;">
				...
				<cplx:tr>
					...
					<cplx:td align="right">
						 <%if(!BigDecimalUtil.isZero(values[0])){ %>
						 	<%=BigDecimalUtil.formatNumberZero(values[0])%>
						 <%} %>
					</cplx:td>
					<cplx:td align="right">
						<%if(!BigDecimalUtil.isZero(values[1])){ %>
							<%=BigDecimalUtil.formatNumberZero(values[1])%>
						<%} %>
					</cplx:td>
					
					<c:set var="stl" value='<%=values[2].compareTo(BigDecimalUtil.ZERO) == 1 ?"green":"red" %>' />
					
					<cplx:td align="right" style="font-weight: bold;color:${stl }">
						 <%if(!BigDecimalUtil.isZero(values[2])){ %>
						 	<%= BigDecimalUtil.formatNumberZero(values[2]) %>
						 <%} %>	
					</cplx:td>
					<cplx:td align="right" style="font-weight: bold;color:${stl }">
					 	<%if(!BigDecimalUtil.isZero(values[3])){ %>
					 		<%= BigDecimalUtil.formatNumberZero(values[3])+" %" %>
					 	<%} %>
					 </cplx:td>
				</cplx:tr>
				<%}
