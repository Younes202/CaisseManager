...
<script type="text/javascript">
	...
	<div class="widget">
		<div class="widget-header bordered-bottom bordered-blue">
			...
			<button type="button" style="margin-top: 3px;margin-right: 3px;" class="btn btn-primary" data-dismiss="modal">
...
<div class="row" style="margin-left: 0px;">
	<table id="table_histo_form" style="width: 100%;">
		<tr>
		...
		<tr>
			<td colspan="4">
				Commande : <b style="font-size: 11px;">${caisseMouvement.ref_commande }</b><br> 
				Date : <b><fmt:formatDate value="${caisseMouvement.date_creation }" pattern="dd/MM/yyyy HH:mm:ss" /></b>
					[${caisseMouvement.is_tovalidate ? '<span style="color:red;font-weight:normal;">NON VALIDEE</span>' : '<span style="color:green;font-weight:normal;">VALIDEE</span>'}]
					<c:if test="${not caisseMouvement.is_tovalidate and caisseMouvement.last_statut == 'ANNUL'}">
	...
	<tr>
		<c:if test="${caisseMouvement.mtt_donne > 0 }">
			<td>Espèces</td>
			<td colspan="3"><fmt:formatDecimal value="${caisseMouvement.mtt_donne }"/></td>
		</c:if>
		<c:if test="${caisseMouvement.mtt_donne_cheque > 0 }">
			<td>Chèque</td>
			<td colspan="3"><fmt:formatDecimal value="${caisseMouvement.mtt_donne_cheque }"/></td>
		</c:if>
		<c:if test="${caisseMouvement.mtt_donne_cb > 0 }">
			<td>Carte</td>
			<td colspan="3"><fmt:formatDecimal value="${caisseMouvement.mtt_donne_cb }"/></td>
		</c:if>
		<c:if test="${caisseMouvement.mtt_donne_dej > 0 }">
			<td>Chèque déj.</td>
			<td colspan="3"><fmt:formatDecimal value="${caisseMouvement.mtt_donne_dej }"/></td>
		</c:if>
	</tr>
	<c:if test="${caisseMouvement.mtt_art_offert > 0 or caisseMouvement.mtt_reduction > 0 }">
		<tr>
			<c:if test="${caisseMouvement.mtt_art_offert > 0 }">
				<td>Offert</td>
				<td colspan="3"><fmt:formatDecimal value="${caisseMouvement.mtt_art_offert }" /> </td>
			</c:if>
			<c:if test="${caisseMouvement.mtt_reduction > 0 }">
				<td>Réduction</td>
				<td colspan="3"><fmt:formatDecimal value="${caisseMouvement.mtt_reduction }"/></td>
			</c:if>	
		</tr>
		<tr><td colspan="4"><hr style="margin-top: 5px;margin-bottom: 0px;"></td></tr>
	</c:if>
	<tr>
		<td>Montant net</td>
		<td colspan="3" style="font-weight: bold;font-size:16px;color: #630767;">
...
<table style="width: 100%;color: black;border: 1px solid #2dc3e8;margin-left: -10px;" id="cmd-table" align="center">
<%
	// Total par menu
...
<br>
			<button type="button" id="close_modal" style="height: 28px;padding-top: 3px;margin-left: 32%;" class="btn btn-lg btn-primary" data-dismiss="modal">
				<i class="fa fa-times"></i> Fermer
			</button>
</div>
</std:form>
