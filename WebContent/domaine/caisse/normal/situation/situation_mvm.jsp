...
<cplx:table name="list_situation" transitionType="simple" width="100%" autoHeight="true" title="Historique des commandes sur la période" initAction="caisse-web.caisseWeb.init_situation_mvm" exportable="false" checkable="false" paginate="false">
	<cplx:header>
	<%if(isHistoriqueShow || isPrintRaz){%>
		<cplx:th type="empty" width="50"/>
	<%} %>
		<cplx:th type="string" value="Commande" field="caisseMouvement.ref_commande" width="210" />
		<%if(isHistoriqueShow || isPrintRaz){%>
		<cplx:th type="decimal" value="Montant" field="caisseMouvement.mtt_commande" width="90"/>
		<cplx:th type="decimal" value="Montant NET" field="caisseMouvement.mtt_commande_net" width="90"/>
		<%if("livr".equals(mnu)){%>
			<cplx:th type="decimal" value="Part livreur" field="caisseMouvement.mtt_livraison_livr" width="90"/>
			<cplx:th type="decimal" value="Part société" field="caisseMouvement.mtt_livraison_livr" width="90" filtrable="false" sortable="false"/>
		<%} %>
		<cplx:th type="decimal" value="Frais livraison" field="caisseMouvement.mtt_livraison_ttl" width="90"/>
		<cplx:th type="decimal" value="Mode paiement" field="caisseMouvement.mode_paiement" width="90" />
		<%} %>
		<cplx:th type="string" value="Statut" align="center" field="caisseMouvement.last_statut"/>
		<cplx:th type="string" value="Call" field="caisseMouvement.num_token_cmd" width="50"/>
		<cplx:th type="string" value="Table" field="caisseMouvement.ref_table" filtrable="false" sortable="false" width="90"/>
		<cplx:th type="string" value="Type" field="caisseMouvement.type_commande"/>
		<%if(isHistoriqueShow || isPrintRaz){%>
			<cplx:th type="empty" width="50"/>
		<%} %>
	</cplx:header>
		...
		<c:forEach items="${listMouvement }" var="mouvement">
			...
			<%} %>
				...
				<%if("livr".equals(mnu)){%>
					...
					<cplx:td align="right" style="font-weight:bold;" value="${mouvement.mtt_livraison_ttl-mouvement.mtt_livraison_livr}"></cplx:td>
				<%} %>
