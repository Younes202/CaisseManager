...
$(document).ready(function (){
	<%if(nbrDayOut != null){%>
		...
		<%if(nbrDayOut >= 15){%>
			...
			document.getElementById("abn_span_count").innerHTML = 'ABONNEMENT EXPIRE';
			
			$("#abn_close_pop").text("FERMER L'ALERTE ").show();
		<%}
	} %>
});
</script>
...
<%
	...
	if(ContextAppli.getUserBean() != null){
		isManagerOrAdmin = (ContextAppli.getUserBean().isInProfile("MANAGER") || ContextAppli.getUserBean().isInProfile("ADMIN"));
	}
...
<div class="page-header position-relative" style="height: 51px;left: 0px;top: -4px;<%=StringUtil.isEmpty(CaisseWebBaseAction.GET_STYLE_CONF("PANEL_ENETETE", null))?"background-image: linear-gradient(#fbfbfb, #ddd)":"background:"+ContextGloabalAppli.getGlobalConfig("PANEL_ENETETE")%>">
	    ...
	    <% if(isJourneeOuverte){ %>	
             ...
             <%} %>                 
                	<% if(isJourneeCaisseOuverte){ %>	
                    	...
                    	<%if(isRestau){ %>
                    	|
                    	<a class="btn btn-default btn-lg shiny btn-top-cai" targetDiv="right-div" style="color:black;text-align:left;" wact="<%=EncryptionUtil.encrypt("caisse.clientMobile.initCmdLigne")%>" params="isrp=0" href="javascript:void(0);" title="les commandes en ligne">
                           ...
                           <ul class="dropdown-menu" id="mnu_drop">
                               ...
                               <%}%>
		                           <%if(StringUtil.isTrueOrNull(ContextGloabalAppli.getGlobalConfig("RIGHT_ACHAT_ART_LIBRE"))
		                            ...
		                            <%if(isManagerOrAdmin && StringUtil.isTrue(ContextGloabalAppli.getGlobalConfig("INVENTAIRE_CAISSE"))){ %>
		                            <li>
		                            	<std:linkPopup action="caisse-web.caisseInventaire.init_inventaire" tooltip="Réaliser un inventaire">
											<i class="fa fa-dropbox" style="color: green;"></i> Réaliser inventaire
										</std:linkPopup>
		                            </li>
		                            <%}%>
		                            <li>
        		...
        		|
       			...
       			<%if(ContextAppli.getUserBean() != null){ %>
       				<span style="left: -72px;
    position: absolute;
    top: 5px;
    font-size: 9px;
    color: red;"><%=ContextAppli.getUserBean().getLogin() %></span>
       			<%} %> 
  		</div>
  ...
  <div class="header-buttons">
  		...
  		<%if(nbrDayOut != null){ %>
  		<a onclick="showPostAlert('', 'Abonnement expiré depuis <%=nbrDayOut%> jours', 'ERROR');" style="position: absolute;right: 316px;padding-left: 0px;height: 50px;padding-top: 14px;" class="blink" title="Expiration abonnement" href="javascript:">
