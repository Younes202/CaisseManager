...
<script type="text/javascript">
	$(document).ready(function() {
		...
		<%if(CURRENT_COMMANDE != null 
				&& CURRENT_COMMANDE.getList_article() != null
				&& CURRENT_COMMANDE.getList_article().size() > 0){ %>
		<% if(StringUtil.isEmpty(CURRENT_COMMANDE.getType_commande())){%>
			$("#send_cuis_pop").show();
			$("#send_cuis").hide();
		<%} else{%>
			$("#send_cuis_pop").hide();
...
<!-- Total -->
	...
	%>
		<span style="position: absolute;left: 232px;top: 0px;">
	...
	<button type="button" id="close_modal" onclick="$('#left-div').hide(500);" 
				style="border-radius: 15px;height:35px; right: 4px;top: 16px;position: absolute;" class="btn btn-danger btn-xs" data-dismiss="modal">
 		   	 ...
 		   	 %> 
		    	   <tr style="color: black;background-color: #f44336;" class="client-root-style" par="iscli=1&idx_cli=<%=i%>&tp=CLI">
			       		<td colspan="3" style="padding-left: 10px;"> 
				       		...
				       		%>
							    <std:link classStyle="btn btn-default btn-sm shiny" style="color:black;" params="<%=param_1 %>" action="caisse-web.caisseWeb.printTicketCaissePersonalise" targetDiv="div_gen_printer" icon="fa fa-check-square-o " tooltip="Imprimer le ticket personnalis&eacute;" />
							<%} %>
		        	       ...
		        	       <%
		        	       	isFamillePassed = true;
		        	       		       ...
		        	       		       String params = (BooleanUtil.isTrue(caisseMvmP.getIs_offert())?"isaf=1&":"") 
		        	       		    		   				...
		        	       		    		   				+ "&tp="+caisseMvmP.getType_ligne()+(caisseMvmP.getId()!=null?"&isDb=1":"");
		        	       		       if(caisseMvmP.getLevel() != null && caisseMvmP.getLevel() > 1){
...
<script type="text/javascript">
	$(document).ready(function (){
		...
		$("#span_cmd").html("<%=(CURRENT_COMMANDE!=null && CURRENT_COMMANDE.getList_article().size()>0?"<i class='fa fa-exchange'></i> DÉTAIL" : "") %>");
