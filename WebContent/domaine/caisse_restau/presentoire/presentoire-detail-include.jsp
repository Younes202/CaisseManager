...
<div style="position: absolute;top: 4px;right: 170px;z-index: 1000;" id="pager-bloc">
  <std:link classStyle="btn btn-blue btn-sm icon-only white" targetDiv="corp-div" action="caisse-web.presentoire.loadCommande" params="<%=prev%>" style="<%=stlPrev%>" icon="fa fa-arrow-circle-left" />
	<span style="color: white;"><%=currNbrElmnt%>/<%=nbrTotal%></span>
  <std:link classStyle="btn btn-blue btn-sm icon-only white" targetDiv="corp-div" action="caisse-web.presentoire.loadCommande" params="<%=next%>" style="<%=stlNext%>" icon="fa fa-arrow-circle-right" />
</div>
<%
	}
%>
      ...
      <div class="row pricing-container" style="padding: 8px;padding-right: 16px;background-color: #424040;">
     <%
	...
	<%
	     ...
	     for(CaisseMouvementPersistant mvm : listCaisseMouvement){ 
	    	 ...
	    	 if(!ContextAppli.STATUT_CAISSE_MOUVEMENT_ENUM.LIVRE.toString().equals(mvm.getLast_statut())){
	...
	%>     
            <div class="plan" style="width: ${Integer.valueOf(100/nbrCols)-2}%;float: left;margin-left: 12px;border-radius: 10px; background-color:<%=colors[idx]%>;">
                <table style="width: 100%;background-color: #FFADBC;border-bottom: 1px;border:none; border-top-right-radius: 10px;border-top-left-radius: 10px;border-collapse: separate; " class="entete-cmd">
                	 <tr style="border-bottom: 1px dashed white;height: 30px;">
                		...
                		<td align="right" style="padding-right: 2px;">
                			...
                			<% if(tp.equals("pre")){ %>
                				<std:link classStyle="btn btn-palegreen shiny btn-xs" style="width: 108px;color:black;" targetDiv="corp-div" params='<%="mnu="+mvm.getId()%>' action="caisse-web.presentoire.changerStatut" icon="fa-3x fa-check" value="Statut" tooltip="Changer le statut de la commande" />
                			<% } else{ %>
		           ...
		           String numCmd = (mvm.getRef_commande().length()>12?mvm.getRef_commande().substring(12):mvm.getRef_commande());     	
		                	%>
		                	<a href="javascript:speak('Commande <%=numCmd %>');" class="btn btn-blue shiny btn-xs" style="width: 80px;color:black;margin-right:10px;"><i class="fa fa-fw fa-file-audio-o"></i> Appeler</a>
                	...
                	<tr style="border-radius: 4px;cursor:pointer;border-bottom: 1px dashed blue;">
                		<td colspan="2" style="text-align: center;">
                			<% if(!ContextAppli.STATUT_CAISSE_MOUVEMENT_ENUM.LIVRE.toString().equals(mvm.getLast_statut()) && isTempsDepasse){ %>
                				<i class="fa fa-warning" style="font-size: 18px;color: red;"></i>
                			<% } %>	
                			<span style="font-size: 18px;font-weight: bold;"><%=numCmd%></span> <br>
             ...
             <div style="overflow: auto;padding-bottom: 10px;" class="div-scoll-cmd">
                <table style="width: 100%;color: black;text-align: left;border-collapse: separate;" id="cmd-table">
					...
					%>
				      	...
				      	<%
 										...
 										if(type.equals(TYPE_LIGNE_COMMANDE.ART.toString()) || type.equals(TYPE_LIGNE_COMMANDE.ART_MENU.toString())){
				      				    	iconStl = "";	 
				      				     }
										...
										%>
						       			...
						       			<div style="margin-top: 4px;float: right;">
							       			<%
							       				if(ContextAppli.STATUT_CAISSE_MOUVEMENT_ENUM.PRETE.toString().equals(caisseMvmP.getLast_statut())){
							       			%>
												<std:link style="" action="caisse-web.presentoire.changerStatut" targetDiv="corp-div" params='<%="cmd="+mvm.getId()+"&mnu="+caisseMvmP.getId()%>' classStyle="btn btn-palegreen shiny btn-xs" icon="fa-3x fa-check" value="" tooltip="Changer statut" />
											<%
												} else{
											%>
